  ...
  if (dayjs().isBefore(dayjs(data.timeAppliedEdit)) && !data.isDel && data.released === 0) {
    return (
      <div>
        ...
        {showPopCancel && <PopConfirmCustom title={`Bạn có muốn tiếp tục huỷ phát hành vé ${data.name} không?`} message={"Vé sẽ không được phát hành theo lịch đã đặt"} handleCancel={(e) => {handleCloseCancelRelease(e)}} handleOk={(e) => {handleAllowCancelRelease(e)}} />}
      </div>
    )
  } else {
    return (
