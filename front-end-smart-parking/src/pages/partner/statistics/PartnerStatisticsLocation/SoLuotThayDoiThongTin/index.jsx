...
const SoLuotThayDoiThongTin = ({ month, year }) => {
  ...
  const query = `SELECT
        COUNT(CASE WHEN location_id IS NULL THEN 1 END) AS so_ve_moi,
        COUNT(CASE WHEN location_id IS NOT NULL THEN 1 END) AS so_dia_diem_chinh_sua
    FROM
        location_modify
    WHERE
        is_del = 0
  ...
  useEffect(() => {
    ...
    getDataStatistic(query).then(response => {
      const result = [{ name: "Thêm mới", value: response.data?.[0].so_ve_moi }, { name: "Chỉnh sửa", value: response.data?.[0].so_dia_diem_chinh_sua }]
      setData(result);
    });
